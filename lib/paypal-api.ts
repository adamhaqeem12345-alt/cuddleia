...
async function getPayPalAccessToken(): Promise<string> {
  ...
  try {
    ...
    if (!response.ok) {
        ...
        throw new Error(`Could not fetch PayPal access token. Status: ${response.status} Details: ${JSON.stringify(errorData, null, 2)}`);
    }
...
export async function captureOrder(orderId: string): Promise<any> {
  ...
  try {
    ...
    const data = await response.json();
    if (!response.ok) {
        throw new Error(`Could not capture PayPal order. Status: ${response.status} Details: ${JSON.stringify(data, null, 2)}`);
    }
...
export async function verifyWebhook(headers: Headers, body: any): Promise<boolean> {
  ...
  try {
    ...
    if (!response.ok) {
        const errorData = await response.json();
        console.error("Error verifying PayPal webhook signature:", JSON.stringify(errorData, null, 2));
        return false;
    }
