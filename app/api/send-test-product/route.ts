...
function createEmailBody(name: string, items: Product[]): string {
    ...
    return `
        <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
            ...
            <h2 style="font-size: 24px; color: #333;">Thank you for your order, ${name}! (Manual Delivery)</h2>
            <p>We're so excited for you to enjoy your new digital goodies. Here is the download link for the item you purchased:</p>
...
export async function GET(req: NextRequest) {
  // Security check: This is a temporary endpoint and should be protected or removed after use.
  // For now, we'll rely on the user to use it once and then ask for its deletion.
  ...
  if (!to || !name) {
    return NextResponse.json({ error: 'Missing required parameters. Please provide "to" (email) and "name".' }, { status: 400 });
  }
  ...
  // Find the specific test product
  const testProduct = products.find(p => p.id === '005');

  if (!testProduct) {
    return NextResponse.json({ error: 'Test product (ID 005) not found.' }, { status: 404 });
  }
  ...
  try {
    ...
    const mailOptions = {
      ...
      to: to,
      subject: 'Your Cuddleia Order Confirmation (Manual Delivery)',
      html: createEmailBody(name, [testProduct]),
    };
