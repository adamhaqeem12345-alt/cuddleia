      ...
      >
        <Card className="flex h-full transform flex-col overflow-hidden rounded-2xl border-none bg-card shadow-lg transition-all duration-300 hover:shadow-2xl hover:-translate-y-2">
          ...
          <CardFooter className="flex items-center justify-between p-6 pt-0">
            ...
            <Button onClick={() => setModalOpen(true)} className="rounded-full bg-primary text-primary-foreground shadow-lg transition-transform hover:scale-105 active:scale-95">
              <Heart className="mr-2 h-5 w-5" /> Buy Now
            </Button>
          </CardFooter>
        </Card>
      </motion.div>
      <PurchaseModal product={product} isOpen={isModalOpen} setIsOpen={setModalOpen} />
    </>
  );
}
